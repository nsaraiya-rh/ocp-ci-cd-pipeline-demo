...
1. Create a branch with **any name**, edit `app/app.py`, commit + push.
2. `build-image` builds the image, tags it `<commit sha>`, pushes to JFrog.
3. `deploy-dev` writes that tag into `gitops/overlays/dev/kustomization.yaml`
   on the `dev` branch (which ArgoCD tracks).
4. ArgoCD auto-syncs `sample-app-dev` within seconds — the dev environment shows
   whatever branch you last pushed (shared "latest push wins").
